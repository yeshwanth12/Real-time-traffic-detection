...
*/
	...
	char* videoFilename ="camera3.asf";
...
Mat croppedImage;
    while( (char)key != 'q' && (char)key != 27 ){
	 ...
	 rectangle( frame2, Point( 0, 0 ), Point( 638,58), Scalar( 0, 0, 0 ), -1, 8 );
	 	 rectangle( frame2, Point( 0, 0 ), Point( 276,187), Scalar( 0, 0, 0 ), -1, 8 );
		  rectangle( frame2, Point( 247,55 ), Point( 403,118), Scalar( 0, 0, 0 ), -1, 8 );
	 imshow("orginal",frame2);
	 		// Setup a rectangle to define your region of interest
	 		RotatedRect rRect = RotatedRect(Point2f(frame2.rows/2,frame2.cols/2), Size2f(300,1000), 75);
			  M = getRotationMatrix2D(rRect.center, 75, 0.5);
			     warpAffine(frame2, rotated, M, frame2.size(), INTER_CUBIC);
				 getRectSubPix(rotated, rRect.size, rRect.center, frame2);
	/*Point2f vertices[4];
rRect.points(vertices);
for (int i = 0; i < 4; i++)
	line(frame2, vertices[i], vertices[(i+1)%4], Scalar(0,255,0));
Rect brect = rRect.boundingRect();
rectangle(frame2, brect, Scalar(255,0,0));
*/

		//    setMouseCallback("orginal", onMouse, 0 );
		if(chk==0)
		{
			frame1=frame2.clone();
			chk=10;
			...
			croppedImage = frame1.clone();
			//rectangle( croppedImage, Point( 0, 0 ), Point( 80,180), Scalar( 0, 0, 0 ), -1, 8 );
			//	rectangle( croppedImage, Point( 240, 0 ), Point( 80,180), Scalar( 0, 0, 0 ), -1, 8 );
	...
	//rectangle( frame2, Point( 0, 0 ), Point( 80, 180), Scalar( 0, 0, 0 ), -1, 8 );
	//rectangle( frame2, Point( 850, 0 ), Point(frame2.rows,frame2.cols), Scalar( 0, 0, 0 ), -1, 8 );
	//rectangle( frame2, Point( 750, 0 ), Point(frame2.rows,frame2.cols), Scalar( 0, 0, 0 ), -1, 8 );
	...
	//	imshow("motion",motion);
       ...
       erode(motion, motion, kernel_ero);
		...
		if(average <1 && area > 40)
			cout<<"VERY HIGH!!"<<endl;
		else if(average >= 0 && average < 800 && (area < 50))
			cout<<"LOW"<<endl;
		else if(average >=800 && average < 2000 || (area >=50 && area <80))
			cout<<"MEDIUM"<<endl;
		else if(average <=100  &&  area >=80  )
			cout<<"HIGH"<<endl;
		next_frame = prev_frame.clone();
